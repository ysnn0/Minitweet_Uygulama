...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate("/home"); // Başarılı girişte /home adresine yönlendir
    } catch (err) {
      console.error("Giriş hatası:", err);
      // Backend'den gelen spesifik hata mesajını göster (eğer varsa)
      setError(err.response?.data?.message || "Giriş yapılamadı. Email veya şifre hatalı olabilir.");
    } finally {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-900 text-gray-100 p-4">
      ...
      <div className="relative z-10 max-w-md w-full bg-gray-800 bg-opacity-80 backdrop-blur-md p-8 rounded-xl shadow-2xl border border-gray-700 transform transition-transform duration-300 hover:scale-[1.005]">
        <h1 className="text-4xl font-extrabold text-blue-400 text-center mb-8 drop-shadow-lg animate-fadeIn">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            className="btn-primary w-full py-3 text-xl bg-blue-600 hover:bg-blue-700 focus:ring-blue-500 transition-all duration-200 ease-in-out"
        ...
        <p className="text-center text-gray-400 mt-6 text-md">
          Hesabın yok mu?{" "}
          <Link to="/register" className="text-blue-400 hover:underline font-semibold transition-colors duration-200">
            Hemen Kaydol!
          </Link>
        </p>
      </div>
